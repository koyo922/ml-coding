...
def init_log(logger_name=None, log_path=None, level=None, when="MIDNIGHT", backup=365,
             is_writing_console=True,
             is_show_logger_src=False, is_propagate=False,
    ...
    assert is_writing_files or is_writing_console, 'Neither writing to file/console, useless logger'
    ...
    if len(logger.handlers) > 0:  # 如果是个已存在的logger对象，直接返回它，不再重复挂载handlers
        logger.warning('[SKIP]you are trying to get a duplicate logger object')
        return logger  # 该问题在Jupyter Notebook中重复运行一个含有`init_log()`代码的cell时容易遇到
